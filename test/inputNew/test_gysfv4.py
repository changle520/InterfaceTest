...
class Test_CheckFile():
    ...
    def test_GYSFV4_NEW_001(self):
        self.test_rlt=check_file(testfile,sheetname[0],'GYSFV4_NEW_001',self.service_code,self.type['ipt'],7)
        print(self.test_rlt)
        self.order_status = [v for k, v in self.test_rlt if k == 'order_status']
        assert self.order_status==[-1]

    def test_GYSFV4_NEW_002(self):
        self.test_rlt=check_file(testfile,sheetname[0],'GYSFV4_NEW_002',self.service_code,self.type['ipt'],7)
        print(self.test_rlt)
        self.order_status = [v for k, v in self.test_rlt if k == 'order_status']
...
class Test_MergeOders():
    ...
    def test_GYSFV4_NEW_004(self):
        self.response_xml = moreorders_havedel(testfile, sheetname[0], 'GYSFV4_NEW_004', self.service_code, 7)
    ...
    def test_GYSFV4_NEW_005(self,update_config_one):
        self.response_xml = moreorders(testfile, sheetname[0], 'GYSFV4_NEW_005', self.service_code, 7)
        self.test_rlt = get_drugnames(self.response_xml)
        print(self.test_rlt)
        self.error_info = [v for k, v in self.test_rlt if k == 'error_info']
    ...
    def test_GYSFV4_NEW_006(self,update_config_one):
        self.response_xml = moreorders(testfile, sheetname[0], 'GYSFV4_NEW_006', self.service_code, 7)
        self.test_rlt = get_drugnames(self.response_xml)
        print(self.test_rlt)
        self.error_info = [v for k, v in self.test_rlt if k == 'error_info']
    ...
    def test_GYSFV4_NEW_007(self,update_config_one):
        self.response_xml = moreorders(testfile, sheetname[0], 'GYSFV4_NEW_007', self.service_code, 7)
        self.test_rlt = get_drugnames(self.response_xml)
        print(self.test_rlt)
    ...
    def test_GYSFV4_NEW_008(self,update_config_one):
        self.response_xml = moreorders(testfile, sheetname[0], 'GYSFV4_NEW_008', self.service_code, 7)
        self.test_rlt = get_drugnames(self.response_xml)
        print(self.test_rlt)
        self.drug_name = [v for k, v in self.test_rlt if k == 'drug_name']
    ...
    def test_GYSFV4_NEW_009(self,update_config_one):
        self.response_xml = moreorders(testfile, sheetname[0], 'GYSFV4_NEW_009', self.service_code, 7)
        self.test_rlt = get_drugnames(self.response_xml)
        print(self.test_rlt)
        self.error_info = [v for k, v in self.test_rlt if k == 'error_info']
        assert "头孢丙烯片与头孢丙烯分散片的作用机制相同。"  in self.error_info
    ...
    def test_GYSFV4_NEW_010(self,update_config_one):
        ...
        self.response_xml = moreorders(testfile, sheetname[0], 'GYSFV4_NEW_010', self.service_code, 7)
        self.test_rlt = get_drugnames(self.response_xml)
        print(self.test_rlt)
        self.error_info = [v for k, v in self.test_rlt if k == 'error_info']
        assert "头孢丙烯片与头孢丙烯分散片的作用机制相同。"  not in self.error_info
